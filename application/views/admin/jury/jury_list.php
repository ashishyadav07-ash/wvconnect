...
<script>
  ...
  var table = $('#na_datatable').DataTable({
    "processing": true,
    "serverSide": false,
    "ajax": "<?=base_url('admin/jury/datatable_json')?>",
    "order": [[0,'asc']],
    "columnDefs": [
...
<script type="text/javascript">
  $("body").on("change", ".tgl_checkbox", function() {
    ...
    $.post('<?=base_url("admin/jury/change_status")?>', {
        ...
        function(data) {
            ...
            if (status === 1) {
                ...
                messageText = 'Data Approved Successfully';
            } else {
                titleText = 'Success notice';
                messageText = 'Data Disapproved Successfully';
            }
            new PNotify({
                title: titleText,
                text: messageText,
...
S
    $.post('<?=base_url("admin/jury/nominee_delete")?>',
    {
      '<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>',
      id : $(this).data('id'),
